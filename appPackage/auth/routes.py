...
from appPackage.auth import bp
from appPackage.auth.forms import LoginForm, RegistrationForm, ResetPasswordRequestForm, ResetPasswordForm
from appPackage.auth.email import send_password_reset_email
...
def login():
    ...
    if form.validate_on_submit():                                           #если данные введены
        ...
        if not next_page or url_parse(next_page).netloc != '':              #если next не установлен или установлен полный путь с именем домена
            next_page = url_for('main.index')                                    #то переходим на страницу index
        return redirect(next_page)
    return render_template('auth/login.html', title=_('Sign In'), form=form)        #генерим страницу при входе впервые
